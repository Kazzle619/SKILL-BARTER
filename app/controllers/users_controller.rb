class UsersController < ApplicationController
  ...
  # 要リファクタリング
  def mypage
    # 今日、自分に申請を出してきている案件一覧を取得する。
    @propositions_offering_to_me_today = Proposition.joins(offering_relationship: :offered).where(
      offers: { created_at: Time.zone.now.all_day },
      offereds_offers: { user_id: current_user.id },
    )
